...
void cancel(bus* bu , int id){
    ...
    if(found){
        swap(&bu->confimed[i], &bu->waitList[0]);
        // add waitlist 1st to confirm list
        // bu->confCount-=1;



        for (int i = *bu->confCount / 2 - 1; i >= 0; i--) {
            heapify(bu->confimed, bu->confCount, i);
        }
    ...
    if(found){
        ...
        swap(&bu->waitList[i], &bu->waitList[*bu->waitCount-1]);
        bu->waitCount--;
